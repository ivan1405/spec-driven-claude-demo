...
Lets a user switch the task board's color scheme between light and dark via
a single header control. The theme is not persisted — a page reload always
...
the theme SHALL NOT be read from or written to `localStorage`, `sessionStorage`,
cookies, or any other persistent store.
