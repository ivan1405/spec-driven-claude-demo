...
violate REQ-003 (no persistence); keeping the state deliberately ephemeral
(component state only) makes non-persistence structural rather than a rule
...
(REQ-006) and needs no changes outside `index.css`. The background covers the
whole window; text and other affected elements are updated accordingly so
they remain readable in both modes.
...
---
Approved-by: Ivan Perez <ivan.perez@parserdigital.com>
