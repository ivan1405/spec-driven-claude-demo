...
- Add a theme toggle button to the header, top-right, that switches the
  whole app between a light and a dark color scheme.
...
- `demo/src/index.css` — add a `[data-theme="dark"]` override block for the
  existing CSS custom properties (`--bg`, `--surface`, `--surface-2`,
  `--line`, `--ink`, `--muted`).
- Tests: `Header.test.jsx` (unit), `demo/e2e/task-board.spec.js` or a new
  e2e spec file (toggle + reload behavior).
